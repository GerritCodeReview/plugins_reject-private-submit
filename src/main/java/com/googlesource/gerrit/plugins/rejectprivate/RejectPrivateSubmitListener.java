...
public class RejectPrivateSubmitListener implements MergeValidationListener {
  ...
  @Override
  public void onPreMerge(Repository repo, CodeReviewCommit commit, ProjectState destProject,
      NameKey destBranch, Id patchSetId, IdentifiedUser caller) throws MergeValidationException {
    if (commit == null || commit.change() == null) {
      log.warn("NULL commit or change in listener while submitting to " + destProject + " on "
          + destBranch + " by " + caller + ": " + commit);
      return;
    }
